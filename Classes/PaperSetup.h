...
int PaperSetup(Node* hand, PdtState &state) {
	// Scene
    ...
    mainState->setAttributeAndModes(material, StateAttribute::MATERIAL|StateAttribute::OVERRIDE);
    mainState->setAttributeAndModes(mainProg, StateAttribute::ON|StateAttribute::OVERRIDE);
	mainState->setMode(GL_BLEND, StateAttribute::OFF|StateAttribute::OVERRIDE);
	...
	finalState->setAttributeAndModes(finalProg, StateAttribute::ON);
